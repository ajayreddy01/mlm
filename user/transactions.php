...
<head>
  ...
  <title>Transactions - Agri Invest</title>
  ...
  <<!-- Sidebar (Desktop - Fixed) -->
    ...
    <main class="flex-1 md:ml-64 p-6"> <!-- Added md:ml-64 -->
          ...
          <div class="relative">
            ...
            <div id="notifMenu"
              ...
              <div class="max-h-60 overflow-y-auto">
                <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">💰 You received
                  ₹500 in Wallet</a>
                <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">✅ Task "Water wheat
                  field" is pending</a>
                <a href="#" class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">🎁 Lucky Draw
                  #124124 starts today</a>
      ...
      <div class="w-full max-w-screen-xl mx-auto px-4 md:px-6 py-6 space-y-6">
        ...
        <section class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-4">
            ...
            <p class="text-2xl md:text-3xl font-extrabold tabular-nums text-green-700 dark:text-green-400">₹ 12,500.00
            </p>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-4">
            <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">Deposit Balance</p>
            <p class="text-2xl md:text-3xl font-extrabold tabular-nums text-green-700 dark:text-green-400">₹ 8,000.00
            </p>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-4">
            <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">Withdraw Balance</p>
            <p class="text-2xl md:text-3xl font-extrabold tabular-nums text-green-700 dark:text-green-400">₹ 3,000.00
            </p>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-4">
            <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">Bonus Balance</p>
            <p class="text-2xl md:text-3xl font-extrabold tabular-nums text-green-700 dark:text-green-400">₹ 1,500.00
            </p>
          </div>
        </section>
        ...
        <section class="bg-white dark:bg-gray-800 rounded-2xl shadow p-4 md:p-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-2">
              <a href="deposit.php" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl text-sm">⬆️
                Deposit</a>
              <a href="withdraw.php"
                class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-xl text-sm">⬇️ Withdraw</a>
            </div>
          </div>
        </section>
        ...
        <section class="flex flex-col sm:flex-row gap-3">
          <input type="text" placeholder="Search by ID or type…"
            class="bg-white dark:bg-gray-800 dark:text-gray-200 rounded-xl shadow px-4 py-2 w-full sm:w-72 focus:outline-none focus:ring-2 focus:ring-green-500">
          <select
            class="bg-white dark:bg-gray-800 dark:text-gray-200 rounded-xl shadow px-4 py-2 w-full sm:w-56 focus:outline-none focus:ring-2 focus:ring-green-500">
        ...
        <section class="space-y-4">
          ...
          <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
            <td class="py-3 px-4">#' . $row['transaction_id'] . '</td>
            <td class="py-3 px-4">' . $row['type'] . '</td>
            <td class="py-3 px-4 text-right tabular-nums font-semibold ' . $amountClass . '">' . $amount . '</td>
            <td class="py-3 px-4">' . $row['created_at'] . '</td>
            <td class="py-3 px-4"><span class="' . $statusClass . ' px-2 py-1 rounded-lg text-xs">' . ucfirst($row['status']) . '</span></td>
          </tr>';
                }
                ?>
              </tbody>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
        <div class="flex items-center justify-between">
          <p class="text-sm text-gray-500 dark:text-gray-400">#' . $row['transaction_id'] . ' • ' . $row['type'] . '</p>
          <span class="' . $statusClass . ' px-2 py-1 rounded text-xs">' . ucfirst($row['status']) . '</span>
        </div>
        <p class="mt-2 text-lg font-semibold ' . $amountClass . '">' . $amount . '</p>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">' . $row['created_at'] . '</p>
      </div>';
            }
            ?>
          </div>
      ...
      <!-- Bottom nav (mobile) -->
      <nav
        class="fixed bottom-0 left-0 right-0 z-20 bg-green-700 dark:bg-green-900 text-white flex justify-around py-3 md:hidden shadow-lg">
