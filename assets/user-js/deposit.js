...
$(function () {
  ...
  if (dt_responsive_table.length) {
    var dt_responsive = dt_responsive_table.DataTable({
      ajax: {
        url: "http://localhost/VKTSR/admin/test/deposit.php", // URL to your PHP script
      ...
      responsive: {
        details: {
          ...
          renderer: function (api, rowIdx, columns) {
            ...
            var transactionId = api.row(rowIdx).data().transaction_id; // Fetch transaction ID for the row
            var buttons = `<div class="mt-3 text-center">
                <button type="button" class="btn btn-success me-2 success-btn" data-id="${transactionId}">Success</button>
                <button type="button" class="btn btn-danger failed-btn" data-id="${transactionId}">Failed</button>
              </div>`;
  ...
  $(document).on("click", ".success-btn, .failed-btn", function () {
    ...
    $.ajax({
      url: "http://localhost/VKTSR/admin/test/update_status.php",
      method: "POST",
      data: {
